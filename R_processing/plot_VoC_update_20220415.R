...
# COVID19 pandemic.
#
# This file is used to update the analysis from April 2022. These results 
# have been described and presented in Technical Note v20220415.
#
...
# SIMULATION DATA
output_dir <- 'data/results_technical_notes/sm24_v20220415/'
...
output_folder <- paste0('output/figures_update_v20220415')
unlink(output_folder, recursive=TRUE)
dir.create(output_folder)
print(output_folder)

# numeric output
csv_output_file <- file.path(output_folder,'technical_note_v20220415_numeric_results.csv')
if(file.exists(csv_output_file)){file.remove(csv_output_file)}

# x-axis limits
x_axis_scm_day <- c(600,850)
x_axis_label <- paste(unique(format(sim_day2date(x_axis_scm_day),'Time (%Y)')),collapse='-')
default_y_axis <- TRUE

# set SCM start date
scm_callibration_date    <- as.Date('2022-04-11')

# number of CoMix and additional waves
scm_num_cp <- 43+8

bool_add_cp    <- TRUE
scm_scenario_cp <- c(as.Date('2022-03-14'),as.Date('2022-03-28'),as.Date('2022-04-11'))
...
col_lib_full <- data.frame(tags = c('xxxx',
                               ...
                               'cnt0_0_110_120_130.*ubooster60_may15', 
                               'cnt0_0_110_140_170.*ubooster60_may15', # ABC-change point label
                               'cnt0_0_110_140_170.*ubooster60_may15', # vaccine uptake label
                               'cnt0_0_110_140_170.*ubooster100_may15',
                               'cnt0_0_0_0.*ubooster50_apr10'),
                      col  = c('black','darkred','darkgreen','orange3','orange3','blue3','darkblue'),
                      label = c(paste0('Reported (',max(be_ref_data$date),')'),
                                'Estimated dynamics, no introductions and reported vaccine uptake',
                                'Changed dynamics: A=110%, B=120%, C=130% of last wave',
                                'Changed dynamics: A=110%, B=140%, C=170% of last wave',
                                'Reported vaccine uptake, with increased transmission dynamics (A=110%, B=140%, C=170%)',
                                'Increased booster dose uptake, with increased transmission dynamics (A=110%, B=140%, C=170%)',
                                'Adjusted historical booster dose uptake by 50%'),
                      lwd = c(NA,2,2,2,2,2,2),
                      pch = c(1,NA,NA,NA,NA,NA,NA))
#bool_file_sort_decreasing <- T

#' if(grepl('cnt0_',output_dir)){
#'   
#'   # set figure legend and color libray
...
# defaults
col_lib_full$day_start <- x_axis_scm_day[1]
col_lib_full$bool_scenario_cp <- TRUE
col_lib_full$bool_file_sort_decreasing  <- TRUE
col_lib_full$wb_id <- paste('scenario',1:nrow(col_lib_full)-1)

col_lib_dynamics <- col_lib_full[c(1:4),]
col_lib_catchup  <- col_lib_full[c(1,5,6),]
col_lib_uptake   <- col_lib_full[c(1,2,7),]


col_lib_catchup$bool_file_sort_decreasing   <- FALSE
col_lib_catchup$day_start                   <- 720

col_lib_uptake$bool_file_sort_decreasing    <- FALSE
col_lib_uptake$bool_scenario_cp             <- FALSE
col_lib_uptake$day_start                    <- 610
...
get_wb_id <- function(file_name,col_lib){
  flag_file <- unlist(lapply(col_lib$tags,grepl,file_name))
  if(!any(flag_file)) {
    warning(paste('NO LABEL TYPE FOR:',file_name))
    return(1)
  ...
  return(col_lib$wb_id[flag_file])
}
...
add_scen_legend <- function(col_lib, opt_files = NA,ref_pch = 1,ref_label = '',ncol = ncol, ref_date = NA){
  ...
  legend(ifelse(min(x_axis_scm_day)==720,'topright','topleft'), # 
...
plot_incidence_time <- function(output_files, 
  ...
  plot(x = plot_ref_data$date,
       ...
       ylim = y_lim,
       xaxt='n',
       main= ifelse(bool_add_title,'Updated figure from Technical Note SIMID v2022-04-15',''),
       xlab=x_axis_label, 
  ...
  if(!is.na(scm_callibration_date)){
    add_vertical_line(scm_callibration_date,bool_text =T,date_tag= 'callibration',pos_factor=1.2)
  }
  abline(h=y_abline,lty=2)
  add_change_points(scm_scenario_cp)

  # set xlsx file name for numeric results
  xlsx_file_name <- file.path(output_folder,paste0(output_tag,'_technical_note_v20220415.xlsx'))
  if(file.exists(xlsx_file_name)){
    wb_aggr <- loadWorkbook(file = xlsx_file_name)
  } else{
    wb_aggr <- createWorkbook()
    wb_legend <- data.frame(id = col_lib_full$wb_id,
  ...
  for(i_file in 1:length(output_files_inc)){
    ...
    scen_wb_id <- get_wb_id(output_files_inc[i_file],col_lib)
    ...
    if(bool_polygon){
      ...
      #print(scen_label)
      if(!scen_wb_id %in% names(wb_aggr)){
        ...
        addWorksheet(wb_aggr, scen_wb_id)
        writeData(wb_aggr, scen_wb_id, hosp_aggr, startRow = 1, startCol = 1)
      }
  ...
  # sort sheets, and save workbook
  worksheetOrder(wb_aggr) <- c(1,order(names(wb_aggr)[-1])+1)
...
sel_region='belgium';file_name='technical_note_v20220415_fig1_dynamics.pdf';col_lib=col_lib_dynamics
get_regional_figures <- function(sel_region,file_name,output_files,col_lib,bool_multi = TRUE){
  ...
  if(bool_multi){
    ...
    pdf(file.path(output_folder,file_name),7,4)
    par(mar=c(4.5,5.1,ifelse(bool_add_title,2,0.5),1),mfrow=c(1,1))
  }
  ...
  output_files_x <- sort(output_files_x,decreasing = any(col_lib$bool_file_sort_decreasing))
  
  be_ref_data_x     <- get_latest_incidence_data(sel_region=sel_region)
  
  if(any(col_lib$bool_scenario_cp)){
    scm_scenario_cp_x <- scm_scenario_cp
  } else{
    scm_scenario_cp_x <- NA
  }
  
  x_axis_scm_day_x <- x_axis_scm_day
  x_axis_scm_day_x[1] <- unique(col_lib$day_start)
  
  sel_region_title <- paste(toupper(substring(sel_region, 1,1)), substring(sel_region, 2), sep="", collapse=" ")
  plot_incidence_time(output_files_x,'/hosp_adm',col_lib = col_lib ,x_axis_scm_day=x_axis_scm_day_x,bool_polygon = T,scm_callibration_date=scm_callibration_date,scm_scenario_cp = scm_scenario_cp_x,be_ref_data=be_ref_data_x)
  #legend('topright',sel_region_title,bg='white')
  if(bool_multi) plot_incidence_time(output_files_x,'/hosp_adm_xtra',col_lib = col_lib ,x_axis_scm_day=x_axis_scm_day_x,bool_polygon = T,scm_callibration_date=scm_callibration_date,scm_scenario_cp = scm_scenario_cp_x,be_ref_data=be_ref_data_x)
  #if(!bool_multi) legend('topright',sel_region_title,bg='white')
  plot_incidence_time(output_files_x,'/hosp_load_incr',col_lib = col_lib ,x_axis_scm_day=x_axis_scm_day_x,bool_polygon = T,scm_callibration_date=scm_callibration_date,scm_scenario_cp = scm_scenario_cp_x,be_ref_data=be_ref_data_x)
  #legend('topright',sel_region_title,bg='white')
  plot_incidence_time(output_files_x,'/icu_load_incr',col_lib = col_lib ,x_axis_scm_day=x_axis_scm_day_x,bool_polygon = T,scm_callibration_date=scm_callibration_date,scm_scenario_cp = scm_scenario_cp_x,be_ref_data=be_ref_data_x)
  #legend('topright',sel_region_title,bg='white')
  ...
  # legend('topright',sel_region_title,bg='white')
  plot_incidence_time(output_files_x,'Rt_mild',col_lib = col_lib ,x_axis_scm_day=x_axis_scm_day_x,bool_polygon = T,scm_callibration_date=scm_callibration_date,scm_scenario_cp = scm_scenario_cp_x,be_ref_data=be_ref_data_x)
  #legend('topright',sel_region_title,bg='white')
...
# ## FIGURES: BE FEBRUARY----
get_regional_figures('belgium','technical_note_v20220415_fig1_dynamics.pdf',output_files = output_files,col_lib=col_lib_dynamics)
get_regional_figures('belgium','technical_note_v20220415_fig1_dynamics_xxx_belgium.pdf',output_files = output_files,col_lib=col_lib_dynamics,bool_multi = FALSE)
# 
get_regional_figures('belgium','technical_note_v20220415_fig2_catchup.pdf',output_files = output_files,col_lib=col_lib_catchup)
get_regional_figures('belgium','technical_note_v20220415_fig2_catchup_xxx_belgium.pdf',output_files = output_files,col_lib=col_lib_catchup,bool_multi = FALSE)
# 
# ## FIGURES: COUNTERFACTUAL----
#get_regional_figures('belgium','technical_note_v20220415_fig3_counterfactual.pdf',output_files = output_files,col_lib=col_lib_uptake)
# get_regional_figures('belgium','technical_note_v20220415_fig3_counterfactual_xxx_belgium.pdf',output_files = output_files,col_lib=col_lib_uptake,bool_multi = FALSE)
 


#COPY FILES TO GDRIVE ----
#update gdrive figures (external)
#gdrive_path <- '~/Google\ Drive/nCov2019\ -\ research/Vaccine/stochastic_model/figures/shared_external/TechNote_v20220706'
...
for(sel_file in output_file_names[grepl('pdf',output_file_names) & !grepl('xxx',output_file_names)]){
