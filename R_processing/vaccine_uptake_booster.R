...
# endpoint_reported_data <- as.Date('2021-11-05')

# endpoint uptake
endpoint_uptake <- as.Date('2022-11-01')
...
# target: based on 1st dose uptake 35-44y on Dec 20th, 2021
# BE: 86%
# BXL: 70%
# FL: 91%
# WL: 80%
...
# sens_uptake_cap  <- 0.90
# sens_uptake_cap_child  <- 0.55 
# 
# sel_region <- "wallonia" # belgium, flanders, wallonia, brussels
# sens_uptake_cap  <- 0.75
# sens_uptake_cap_child  <- 0.20
# 
# sel_region <- "brussels" # belgium, flanders, wallonia, brussels
# sens_uptake_cap  <- 0.50
# sens_uptake_cap_child  <- 0.10

sel_region <- "belgium" # belgium, flanders, wallonia, brussels
sens_uptake_cap        <- 0.80
sens_uptake_cap_child  <- 0.40
sens_uptake_booster    <- 0.6

#timing_booster_finished <- as.Date('2022-5-15')
timing_booster_finished <- as.Date('2022-05-01')
...
scen_out_dose3_aggr <- scen_out_final[42:41]
...
legend('topleft',
       c('Uptake: total',
         ...
         'Model: 1st dose',
         'Model: 2nd dose'),
       col=c(1,4,5,7,NA,8,8,8,8,8),
       lty=c(1,1,1,1,NA,NA,NA,NA,1,3),
       pch=c(NA,NA,NA,NA,NA,1,3,6,NA,NA),
       lwd=2,
       xpd=TRUE,
       cex=1.1)
...
plot(1,bty="n",yaxt="n",xaxt="n",xlab='',ylab='',col=0)
legend('topleft',
       c(paste('Period:',paste(range(scen_out_final$date[sel_jan2022]),collapse=' to ')),
         ...
         '',
         paste0('End booster campaign: ',timing_booster_finished)
         ),
