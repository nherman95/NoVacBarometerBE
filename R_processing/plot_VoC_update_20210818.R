...
# COVID19 pandemic.
#
# This file is used to update the analysis from August 2021. These results have
# been described and presented in Technical Note v20210818.
#
...
# SIMULATION DATA
output_dir <- 'data/results_technical_notes/sm18_n20/'
...
output_folder <- paste0('output/figures_update_v20210818')
unlink(output_folder, recursive=TRUE)
dir.create(output_folder)
print(output_folder)

# numeric output
csv_output_file <- file.path(output_folder,'technical_note_v20210818_numeric_results.csv')
...
scm_callibration_date    <- as.Date('2021-08-16')
...
plot_incidence_time <- function(output_files, 
  ...
  plot(x = plot_ref_data$date,
       ...
       ylim = y_lim,
       xaxt='n',
       main= 'Updated figure from Technical Note SIMID v2021-08-18',
       xlab=x_axis_label, 
  ...
  if(!is.na(scm_callibration_date)){
    add_vertical_line(scm_callibration_date,bool_text =T,date_tag= 'callibration',pos_factor=1.3)
  }
  ...
  for(i_file in 1:length(output_files_inc)){
    ...
    if(bool_polygon){
      ...
      # names(hosp_aggr) <- gsub("admin_",paste0(scen_label,' '),names(hosp_aggr))
      # names(hosp_aggr) <- gsub("admin_",paste0(scen_col,' '),names(hosp_aggr))
      # write.table(hosp_aggr,
      #             file = paste0(output_folder,'/',
      #                           output_tag,'_',scen_col,'.csv'),
      #             sep=',',
      #             row.names=F)
      names(hosp_aggr) <- gsub("admin_",'',names(hosp_aggr))
      addWorksheet(wb_aggr, scen_col)
      writeData(wb_aggr, scen_col, hosp_aggr[,c(4,1:3)], startRow = 1, startCol = 1)
  ...
  wb_legend <- data.frame(color = col_lib$col,
                          label = col_lib$label)
  addWorksheet(wb_aggr, "legend")
  writeData(wb_aggr, "legend", 
            wb_legend[2:5,], startRow = 1, startCol = 1)
  writeData(wb_aggr, "legend", 
            c('LL: lower limit',
              'UL: upper limit'), startRow = 10, startCol = 1)
  
  saveWorkbook(wb_aggr, 
               file = file.path(output_folder,paste0(output_tag,'.xlsx')), 
               overwrite = TRUE)
...
## FIGURE 6 ----
pdf(file.path(output_folder,paste0('technical_note_v20210818_fig6.pdf')),9,12)
par(mar=c(4.5,4.5,2,0.5),mfrow=c(3,1))

plot_incidence_time(output_files,'/hosp_adm',col_lib = col_lib ,x_axis_scm_day=x_axis_scm_day,bool_polygon = T,scm_callibration_date=scm_callibration_date)
plot_incidence_time(output_files,'/icu_load',col_lib = col_lib ,x_axis_scm_day=x_axis_scm_day,bool_polygon = T,scm_callibration_date=scm_callibration_date)
plot_incidence_time(output_files,'Rt_mild',col_lib = col_lib ,x_axis_scm_day=x_axis_scm_day,bool_polygon = T,scm_callibration_date=scm_callibration_date)

dev.off()

## SINGLE PLOT FIGURES ----
pdf(file.path(output_folder,paste0('technical_note_v20210818_fig6_bis.pdf')),9,4)
par(mar=c(4.5,4.5,2,0.5),mfrow=c(1,1))
plot_incidence_time(output_files,'/hosp_adm',col_lib = col_lib ,x_axis_scm_day=x_axis_scm_day,bool_polygon = T,scm_callibration_date=scm_callibration_date)
plot_incidence_time(output_files,'/hosp_load',col_lib = col_lib ,x_axis_scm_day=x_axis_scm_day,bool_polygon = T,scm_callibration_date=scm_callibration_date)
plot_incidence_time(output_files,'/icu_load',col_lib = col_lib ,x_axis_scm_day=x_axis_scm_day,bool_polygon = T,scm_callibration_date=scm_callibration_date)
plot_incidence_time(output_files,'Rt_mild',col_lib = col_lib ,x_axis_scm_day=x_axis_scm_day,bool_polygon = T,scm_callibration_date=scm_callibration_date)
plot_incidence_time(output_files,'/new_infect',col_lib = col_lib ,x_axis_scm_day=x_axis_scm_day,bool_polygon = T,scm_callibration_date=scm_callibration_date)
plot_incidence_time(output_files,'/new_infect_xtra',col_lib = col_lib ,x_axis_scm_day=x_axis_scm_day,bool_polygon = T,scm_callibration_date=scm_callibration_date)
dev.off()

# plot and get numerical results for hospital load (FAGG)
#plot_incidence_time(output_files,'/hosp_load',col_lib = col_lib ,x_axis_scm_day=x_axis_scm_day,bool_polygon = T)

## merge numerical results hosp load
#hosp_load_files <- dir(output_folder,full.names = T,pattern = 'hosp_load')


## COPY FILES TO GDRIVE ----
# update gdrive figures (external)
gdrive_path <- '~/Google\ Drive/nCov2019\ -\ research/Vaccine/stochastic_model/figures/shared_external/TechNote_v20210816 (BE)'
...
output_file_names <- dir(output_folder,full.names = F,pattern = '.pdf')
