...
timing_booster_finished <- as.Date('2022-12-15')
timing_booster_start <- as.Date('2022-09-15')

# recurrent campaign every x months
rec_campaign <- 1000

# endpoint uptake
endpoint_uptake <- as.Date('2032-09-10')
...
sens_uptake_extrabooster_old    <- 0.75 # !!! this is absolute, not relative to previous booster
...
# UPTAKE EXTRABOOSTER DOSE  (bi-)annual vaccine campaign ---- 
###################### #
...
scen_out_extrabooster <- merge(scen_out_2ndbooster,
                             ...
                             all = T)
scen_out_resset <- merge(scen_out_2ndbooster,
                              scen_out_resset,
                              all = T)
