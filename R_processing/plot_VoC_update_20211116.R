...
# SIMULATION DATA
output_dir <- 'data/results_technical_notes/sm22_v20211116/'
...
output_folder <- paste0('output/figures_update_v20211116')
unlink(output_folder, recursive=TRUE)
dir.create(output_folder)
print(output_folder)

# numeric output
csv_output_file <- file.path(output_folder,'technical_note_v20211116_numeric_results.csv')
if(file.exists(csv_output_file)){file.remove(csv_output_file)}

# x-axis limits
x_axis_scm_day <- c(550,700)
x_axis_label <- paste(unique(format(sim_day2date(x_axis_scm_day),'Time (%Y)')),collapse='-')
default_y_axis <- TRUE

# set SCM start date
scm_start_date   <- as.Date('2020-03-01')
scm_callibration_date    <- as.Date('2021-11-15')

# number of CoMix and additional waves
scm_num_cp <- 33+8

bool_add_cp    <- TRUE
scm_scenario_cp <- c(as.Date('2021-11-22') )
...
col_lib <- data.frame(tags = c('xxxx',
                      ...
                      label = c(paste0('Reported (',max(be_ref_data$date),')'),
                                ...
                                'Rt(infections) remains stable until November 21st, 2021',
                                'Rt(infections) remains stable until December 2021',
                                'Rt(infections) decreases fast after November 21st, 2021',
...
plot_incidence_time <- function(output_files, 
  ...
  if(any(grepl('hosp_load',output_files_inc))){
    ...
    y_lim     <- c(1,ifelse(min(x_axis_scm_day)==0,7500,5500)) 
    ref_pch   <- 2 
  ...
  if(any(grepl('Rt_hosp',output_files_inc))){
    ...
    y_lim     <- c(0.6,1.7) 
    ref_pch   <- 6
    ref_label <- 'Rt ~ hospital admissions'
    y_abline  <- 1
  }
  ...
  plot(x = plot_ref_data$date,
       ...
       ylim = y_lim,
       xaxt='n',
       main= 'Updated figure from Technical Note SIMID v2021-11-16',
       xlab=x_axis_label, 
  ...
  saveWorkbook(wb_aggr, 
               file = file.path(output_folder,paste0(output_tag,'_technical_note_v20211116.xlsx')), 
               overwrite = TRUE)
}
...
get_regional_figures <- function(sel_region,file_name,output_files,col_lib,bool_multi = TRUE){
  ...
  if(!bool_multi){
    ...
    legend('topright',sel_region_title,bg='white')
    plot_incidence_time(output_files_x,'/new_infect_xtra',col_lib = col_lib ,x_axis_scm_day=x_axis_scm_day,bool_polygon = T,scm_callibration_date=scm_callibration_date,scm_scenario_cp = scm_scenario_cp,be_ref_data=be_ref_data_x)
    legend('topright',sel_region_title,bg='white')
  }
  dev.off()
}
...
## FIGURE 5: BE ----
get_regional_figures('belgium','technical_note_v20211116_fig5bis_belgium.pdf',output_files = output_files,col_lib=col_lib)
get_regional_figures('belgium','technical_note_v20211116_xxx_belgium.pdf',output_files = output_files,col_lib=col_lib,bool_multi = FALSE)


# COPY FILES TO GDRIVE ----
#update gdrive figures (external)
gdrive_path <- '~/Google\ Drive/nCov2019\ -\ research/Vaccine/stochastic_model/figures/shared_external/TechNote_v20211116'
...
for(sel_file in output_file_names[grepl('fig5',output_file_names) | grepl('icu_load',output_file_names)]){
