...
# COVID19 pandemic.
#
# This file is used to update the analysis from August 2021. These results 
# have been described and presented in Technical Note v20210823, which was  
# a supplement of the Belgian analysis described in Techical Note v20210818.
#
...
# SIMULATION DATA
output_dir <- 'data/results_technical_notes/sm18_n20/bxl/'
...
output_folder <- paste0('output/figures_update_v20210823')
unlink(output_folder, recursive=TRUE)
dir.create(output_folder)
print(output_folder)

# numeric output
csv_output_file <- file.path(output_folder,'technical_note_v20210823_numeric_results.csv')
...
scm_callibration_date    <- as.Date('2021-08-20')
...
plot_incidence_time <- function(output_files, 
  ...
  if(any(grepl('hosp_adm',output_files_inc))){
    ...
    ref_pch      <- 1
    y_lim        <- c(0,ifelse(min(x_axis_scm_day)==0,800,200))
    y_abline     <- NA
  }
  ...
  if(any(grepl('hosp_load',output_files_inc))){
    ...
    y_lim     <- c(1,ifelse(min(x_axis_scm_day)==0,7500,2500)) 
    ref_pch   <- 2 
  ...
  plot(x = plot_ref_data$date,
       ...
       ylim = y_lim,
       xaxt='n',
       main= 'Updated figure from Technical Note SIMID v2021-08-23 (BXL)',
       xlab=x_axis_label, 
...
## FIGURE 6 ----
pdf(file.path(output_folder,paste0('technical_note_v20210823_fig1.pdf')),9,12)
par(mar=c(4.5,4.5,2,0.5),mfrow=c(3,1))

plot_incidence_time(output_files,'/hosp_adm',col_lib = col_lib ,x_axis_scm_day=x_axis_scm_day,bool_polygon = T,scm_callibration_date=scm_callibration_date,be_ref_data=be_ref_data)
plot_incidence_time(output_files,'/icu_load',col_lib = col_lib ,x_axis_scm_day=x_axis_scm_day,bool_polygon = T,scm_callibration_date=scm_callibration_date,be_ref_data=be_ref_data)
plot_incidence_time(output_files,'Rt_mild',col_lib = col_lib ,x_axis_scm_day=x_axis_scm_day,bool_polygon = T,scm_callibration_date=scm_callibration_date,be_ref_data=be_ref_data)

dev.off()

## SINGLE PLOT FIGURES ----
pdf(file.path(output_folder,paste0('technical_note_v20210823_fig_bis.pdf')),9,4)
par(mar=c(4.5,4.5,2,0.5),mfrow=c(1,1))

plot_incidence_time(output_files,'/hosp_adm',col_lib = col_lib ,x_axis_scm_day=x_axis_scm_day,bool_polygon = T,scm_callibration_date=scm_callibration_date,be_ref_data=be_ref_data)
plot_incidence_time(output_files,'/icu_load',col_lib = col_lib ,x_axis_scm_day=x_axis_scm_day,bool_polygon = T,scm_callibration_date=scm_callibration_date,be_ref_data=be_ref_data)
plot_incidence_time(output_files,'Rt_mild',col_lib = col_lib ,x_axis_scm_day=x_axis_scm_day,bool_polygon = T,scm_callibration_date=scm_callibration_date,be_ref_data=be_ref_data)
plot_incidence_time(output_files,'/hosp_load',col_lib = col_lib ,x_axis_scm_day=x_axis_scm_day,bool_polygon = T,scm_callibration_date=scm_callibration_date,be_ref_data=be_ref_data)

dev.off()

## COPY FILES TO GDRIVE ----
# update gdrive figures (external)
 gdrive_path <- '~/Google\ Drive/nCov2019\ -\ research/Vaccine/stochastic_model/figures/shared_external/TechNote_v20210823 (BXL)'
