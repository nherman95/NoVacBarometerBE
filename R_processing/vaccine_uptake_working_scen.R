...
scen_num <- cl_val[1]
bool_bivalent_fall2023 <- as.logical(cl_val[2])
} else{
  scen_num <- 3
  bool_bivalent_fall2023 <- TRUE
}
print(scen_num)
print(bool_bivalent_fall2023)
...
timing_booster_start <- as.Date('2023-09-15')
...
# population data
 ...
 sens_flu_pop[7] <- plus65fraction*sens_flu_pop[7]
...
# UPTAKE EXTRABOOSTER DOSE (dedicated bivalent booster) ----
###################### #
...
scen_out_resset <- scen_out_fallcampaign
scen_out_resset2 <- scen_out_fallcampaign
...
#  scen_out_resset2 <- scen_out_resset
  scen_out_resset2[scen_out_resset2$date >= (new_timing_booster_start),paste0('rna_X_',age_opt[i_age])]  <- 0
  scen_out_resset[scen_out_resset$date <= (new_timing_booster_start),paste0('rna_X_',age_opt[i_age])]  <- 0
  
  }
  else{
    scen_out_resset2[scen_out_resset2$date >= (new_timing_booster_start),paste0('rna_F_',age_opt[i_age])]  <- 0
    scen_out_resset[scen_out_resset$date <= (new_timing_booster_start),paste0('rna_F_',age_opt[i_age])]  <- 0
  }
}
if(bool_novac){
  scen_out_fallcampaign[!is.na(scen_out_fallcampaign)] <- 0
  scen_out_fallcampaign$date <- scen_out_2ndbooster$date
}
...
for(i_age in 10:1){
  ...
  lines(scen_out_final$date+1,
          ...
          lty=1,
          lwd=2)
  lines(scen_out_resset2$date+1,
        cumsum(scen_out_resset2[,paste0('rna_X_',age_opt[i_age])]),
        col=6,
        lty=1,
        lwd=2)
  lines(scen_out_resset$date+1,
    ...
    lines(scen_out_resset2$date+1,
          cumsum(scen_out_resset2[,paste0('rna_F_',age_opt[i_age])]),
          col=2,
          lty=1,
          lwd=2)
    lines(scen_out_final$date+1,
...
legend('topleft',
       c('Uptake: total',
         ...
         'Uptake: new booster (XBB1.5 monovalent)',
         '',
         'Reported: 1st dose',
         'Reported: 2nd doses',
         ...
         'Model: 1st dose',
         'Model: 2nd dose'),
       col=c(1,4,5,7,2,6,"#7D26CD",NA,8,8,8,8,8,8),
       lty=c(1,1,1,1,1,1,1,NA,NA,NA,NA,NA,1,3),
       pch=c(NA,NA,NA,NA,NA,NA,NA,NA,1,3,6,9,2,NA,NA),
       lwd=2,
       xpd=TRUE,
       cex=1)} else{
  legend('topleft',
