...
# set scenario tag
scen_tag <- 'nocampaign130922'
#scen_tag <- '65pall280822'

#timing_booster_finished <- as.Date('2022-5-15')
timing_booster_finished <- as.Date('2022-09-30')
timing_booster_start <- as.Date('2022-09-12')

# endpoint uptake
endpoint_uptake <- as.Date('2023-01-15')
...
sens_uptake_extrabooster    <- 0 # 0.5 #0.5 #0.6
...
ref_data_be      <- get_observed_incidence_data(enable_data_reuse = F,
...
dose_col_extrabooster <- paste0('X_',agegroup_opt)
...
db_uptake_2ndbooster <- db_uptake[,grepl('_F_',names(db_uptake))| grepl('date',names(db_uptake))] 
db_uptake_extrabooster <- db_uptake[,grepl('_F_',names(db_uptake))| grepl('date',names(db_uptake))]   #temporary until real data
names(db_uptake_extrabooster) <- gsub('_F_','_X_',names(db_uptake_extrabooster))
db_uptake_extrabooster[-1] <- 0
...
# merge
scen_out_2ndbooster <- merge(scen_out_booster,
                          scen_out_2ndbooster,
                          all = T)
...
# UPTAKE EXTRABOOSTER DOSE ----
###################### #

# define full uptake (from 1st booster)
scen_out_latest <- scen_out_booster[,grepl('_E_',names(scen_out_booster)) | grepl('date',names(scen_out_booster))]
names(scen_out_latest)
...
i_col <- 12
#first campaign
sel_extrabooster_days <- scen_out_extrabooster$date >= timing_booster_start & scen_out_extrabooster$date < (as.Date('2022-09-30')-1)
num_time_steps3 <- sum(sel_extrabooster_days)
if(num_time_steps3 > 0){ # implement future booster campaign
  age_opt     <- get_age_groups()
  s_age <- age_opt[7]
  for(s_age in age_opt[7:10]){ #age_opt[7:10] age_opt[2:10]
    booster_total <- sens_uptake_extrabooster*sum(scen_out_booster[,grepl(paste0('E_',s_age),names(scen_out_booster))])
    booster_total <- max(0,booster_total)
    if(s_age == age_opt[7]){
      booster_total <- booster_total/2
    }
    ...
    print(paste('avg extrabooster doses / day:',round(sens_uptake_extrabooster*booster_total/ num_time_steps3),' for age ',s_age))
  }
} 
#second campaign
# sel_extrabooster_days <- scen_out_extrabooster$date >= as.Date('2022-10-01') & scen_out_extrabooster$date < (as.Date('2022-10-31')-1)
# num_time_steps3 <- sum(sel_extrabooster_days)
# if(num_time_steps3 > 0){ # implement future booster campaign
#   age_opt     <- get_age_groups()
#   s_age <- age_opt[7]
#   for(s_age in age_opt[6:7]){ #age_opt[7:10] age_opt[2:10]
#     booster_total <- sens_uptake_extrabooster*sum(scen_out_booster[,grepl(paste0('E_',s_age),names(scen_out_booster))])
#     booster_total <- max(0,booster_total)
#     if(s_age == age_opt[7]){
#       booster_total <- booster_total/2
...
#third campaign
# sel_extrabooster_days <- scen_out_extrabooster$date >= as.Date('2022-11-01') & scen_out_extrabooster$date < (timing_booster_finished-1)
# num_time_steps3 <- sum(sel_extrabooster_days)
# if(num_time_steps3 > 0){ # implement future booster campaign
#   age_opt     <- get_age_groups()
#   s_age <- age_opt[7]
#   for(s_age in age_opt[2:5]){ #age_opt[7:10] age_opt[2:10]
#     booster_total <- sens_uptake_extrabooster*sum(scen_out_booster[,grepl(paste0('E_',s_age),names(scen_out_booster))])
#     booster_total <- max(0,booster_total)
#     scen_out_extrabooster[sel_extrabooster_days,grepl(paste0('X_',s_age),names(scen_out_extrabooster))] <- booster_total/num_time_steps3
#     print(paste('avg extrabooster doses / day:',round(sens_uptake_extrabooster*booster_total/ num_time_steps3),' for age ',s_age))
...
# merge
scen_out_extrabooster <- merge(scen_out_2ndbooster,
...
scen_out_final <- scen_out_extrabooster
...
legend('topleft',
       ...
       pch=c(NA,NA,NA,NA,NA,NA,NA,1,3,6,9,NA,NA),
       lwd=2,
       xpd=TRUE,
       cex=1.1)
...
legend('topright',
       c(#paste('Period:',paste(range(scen_out_final$date),collapse=' to ')),
         ...
         #'',
         'Extra booster campaign',
         paste0('Start: ',timing_booster_start),
         paste0('End: ',timing_booster_finished),
         paste0('Public: 65+'),
         paste0('Uptake: ',sens_uptake_extrabooster*100,'% of 1st booster dose')
       ),
