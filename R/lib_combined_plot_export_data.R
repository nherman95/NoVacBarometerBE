...
plot_incidence_time <- function(output_files, 
  ...
  if(any(grepl('hosp_adm',output_files_inc))){
    ...
    ref_pch      <- 1
    y_lim        <- c(0,ifelse(min(x_axis_scm_day)==0,800,400))  #700
   # y_abline     <- 100
  }
  ...
  if(any(grepl('new_infect_xtra',output_tag))){
    ...
    ref_pch      <- 5 
    y_lim    <- c(1e7,ifelse(min(x_axis_scm_day)==0,3e7,3e7))
    default_y_axis <- TRUE
  }
  ...
  if(any(grepl('hosp_load',output_files_inc))){
    ...
    y_lim     <- c(1,ifelse(min(x_axis_scm_day)==0,7500,3000)) 
    ref_pch   <- 1#2 
  ...
  write.csv(plot_ref_data$date,"daterep.csv")
  write.csv(plot_ref_data$reported,"reported.csv")
  ...
  if(bool_polygon){
     ...
     # if(any(grepl('hosp_adm_xtra',output_tag))){
      ...
      add_polygon(scenario_data  = hosp_aggr[-nrow(hosp_aggr),],
                  ...
                  mean_lty = ref_pch)
      print(output_files_inc[i_file])
      write.csv(scm_dates,'datescen.csv')
      write.csv(hosp_aggr,paste(i_file,'test.csv'))
