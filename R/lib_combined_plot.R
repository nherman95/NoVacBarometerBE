...
plot_incidence_time <- function(output_files, 
 ...
 #print(output_files_inc)
  ...
  if(any(grepl('hosp_adm',output_files_inc))){
    ...
    ref_pch      <- 1
    y_lim        <- c(0,ifelse(min(x_axis_scm_day)==0,1000,400))  #700
    y_abline     <- c(new_hosp_or,new_hosp_red)
  }
  ...
  if(any(grepl('scen_new_infect',output_files_inc))){
    plot_ref_data <- data.frame(date     = be_ref_data$date,
  ...
  if(any(grepl('new_sympt',output_files_inc))){
    ...
    ref_pch      <- 5 
    y_lim    <- c(0,ifelse(min(x_axis_scm_day)==0,8e4,4e4))
  ...
  if(any(grepl('new_infect_xtra',output_tag))){
    ...
    ref_pch      <- 5 
    y_lim    <- c(1e6,ifelse(min(x_axis_scm_day)==0,1e7,1e7))
    default_y_axis <- TRUE
  }
  ...
  if(any(grepl('scen_mortality_xtra',output_tag))){
    ...
    ref_label    <- 'deaths cumulative (incl. nursing home)' 
    ref_pch      <- 5 #4
    y_lim    <- c(0,ifelse(min(x_axis_scm_day)==0,1e5,1e5))
    default_y_axis <- TRUE
  }
  # if(any(grepl('scen_mortality_xtra',output_tag))){
  #   plot_ref_data <- data.frame(date     = be_ref_data$date,
  ...
  if(any(grepl('hosp_load',output_files_inc))){
    ...
    y_lim     <- c(1,ifelse(min(x_axis_scm_day)==0,8000,3000)) 
    ref_pch   <- 1#2 
    ref_label <- 'hospital load'
  }
  if(any(grepl('icu_load_incr',output_files_inc))){
    plot_ref_data <- data.frame(date     = be_ref_data$date,
  ...
  if(any(grepl('social_coef',output_files_inc))){
    plot_ref_data <- data.frame(date     = be_ref_data$date,
                                reported = rep(1,length(be_ref_data$date)))
    y_lim     <- c(0,1.2) 
    ref_pch   <- 6
    ref_label <- 'Barometer coef'
    y_abline  <- c(red_prop, or_prop)
  }
...
#temp removed !!!!!!!!
  ...
  if(bool_polygon){
      hosp_aggr <-  data.frame(admin_mean=apply(scen_data,1,median),
                               #admin_mean=NA,
